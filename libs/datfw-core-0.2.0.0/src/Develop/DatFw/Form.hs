...
module Develop.DatFw.Form
    (-- * Form construction
      AForm
    , FieldSettings(..), mkSettings, withAttribute, withPlaceholder
...
instance IsString FieldSettings where
    fromString label = mkSettings (T.pack label)

-- | Initial settings with the label.
mkSettings :: Text -> FieldSettings
mkSettings label = FieldSettings label Nothing Nothing Nothing []

-- | Add an attribute to a field.
withAttribute :: Text -> Text -> FieldSettings -> FieldSettings
withAttribute name value fs =
    fs{ fsAttrs = (name, value) : fsAttrs fs }

-- | Add a placeholder attribute to a field.
withPlaceholder :: Text -> FieldSettings -> FieldSettings
withPlaceholder = withAttribute "placeholder"
...
fhelper field fs def onMissing onFound isReq = AForm $ \ mbenv s -> do
    ...
    (res, val) <- case mbenv of
        ...
        Just env -> do
            ...
            case emx of
                Left inv -> pure ( FormFailure [inv], Left $ maybe "" fst $ uncons mvals )
