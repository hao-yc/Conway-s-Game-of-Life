...
import           Network.Wai.Request as W (appearsSecure)
import           Network.Wai.Middleware.Approot as W (getApprootMay)
...
class HasRoute a => WebApp a where
    -- | An absolute URL to the root of the application. Do not include trailing slash.
    --
    -- Default value: 'guessApproot'.
    -- If this is not true or you know your application root
    -- statically, you should override with a different implementation.
    appRoot :: a -> Request -> Text
    appRoot _ req = guessApproot req
    ...
    cleanPath site segs =
      ...
      where
        filterNulls [] = (False, [])
        filterNulls (segsh : segst) =
            let tail2 = filterNulls segst
            in if T.length segsh == 0 then (True, snd tail2)
               else (fst tail2, segsh : snd tail2)
    ...
    --
    -- Default value: all requests are authorized.
...
-- | Guess the approot based on the middleware configuration or on request headers.
-- For more information, see "Network.Wai.Middleware.Approot"
--
-- In the case of headers being unavailable, it falls back to ""
guessApproot :: Request -> Text
guessApproot req =
    let authority =
            maybe "" (mappend (if W.appearsSecure req then "https://" else "http://")) $ W.requestHeaderHost req
        appPath =
            maybe "" id $ lookup "cgi-script-name" (W.requestHeaders req)
        url = maybe (mappend authority appPath) id $ W.getApprootMay req
    in T.decodeUtf8With T.lenientDecode url
...
-- | The default session backend for 'makeSessionBackend'.
