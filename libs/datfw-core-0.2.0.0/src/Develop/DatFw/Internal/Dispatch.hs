...
dispatchHandler :: (WebApp a, ToTypedContent r)
                => HandlerFor a r -> DispatchEnv a -> Maybe (Route a)
                -> Application
dispatchHandler handler env croute request respond = do
    ...
    let ar = appRoot site request
        ...
        onError :: ResponseError -> Request -> SessionMap -> IO HandlerResp
        onError err =
...
runHandler rhenv handler request session = do
    ...
    result <- ((Right . toTypedContent) <$> runReaderT (unHandlerFor handler) hdata)
        `catch` \ e -> pure (Left (toHCException e))
